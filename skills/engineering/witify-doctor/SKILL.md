...
**0.5 The `diagram-design` companion plugin, wherever a harness is present.** It is the third-party plugin the `witify-docx` skill (shipped by the private `witify/skills-admin` plugin, admin staff only) draws its figures with, so a harness without it can still write the document but not the diagrams inside it. Claude Code: a `"diagram-design@diagram-design"` key in `~/.claude/plugins/installed_plugins.json` (fallback: the directory `~/.claude/plugins/cache/diagram-design/diagram-design` exists) and `enabledPlugins["diagram-design@diagram-design"]` in `~/.claude/settings.json`. Codex: `[plugins."diagram-design@diagram-design"] enabled = true` in `config.toml`, with the copy under `~/.codex/plugins/cache/diagram-design/`. Installed and enabled on every harness present is `Healthy`; missing or disabled on one is `Attention`, never `Blocked`, since only `witify-docx` needs it, and only staff who have it. When the plugin is installed, also prove its PNG export can run: `python3 -c "import playwright"` succeeds and `~/Library/Caches/ms-playwright/` (macOS) or `~/.cache/ms-playwright/` (Linux) contains a `chromium*` folder; either missing is `Attention` with the fix `pip install playwright && playwright install chromium`. When the plugin is installed, the Witify brand profile should also be present at `~/.diagram-design/profiles/witify.md`, checked for presence only, never for contents; absent is `Attention`, with the note that any `witify-docx` build installs it automatically (the build script copies it from the skill's `template/diagram-profile/witify.md`), so the fix is simply to build a document once, or to copy that file by hand. Recommended: installed on every harness present. Claude Code: `/plugin marketplace add cathrynlavery/diagram-design` then `/plugin install diagram-design@diagram-design`; Codex: `codex plugin marketplace add cathrynlavery/diagram-design` then `codex plugin add diagram-design@diagram-design`. The full install text lives under "Recommended Companions" in the `witify/skills` README.
