...
4. **When the PR is up and human review comes back** → **`/fix-review`** (`/ship` handles CI and Codex under its selected fix policy, replying to and resolving Codex's own threads; human comments belong to `/fix-review`). It triages every open comment, confirms the approach per concern with the user (numbered questions with a recommended option, grilling-style), then works each to a conclusion — fixes committed per concern and pushed, questions answered, bad suggestions pushed back on with evidence — and replies to each comment with a commit link. It never resolves threads; reviewers close their own. `/code-review` produces a review, `/fix-review` consumes one.
