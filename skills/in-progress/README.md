...
- **[afk-loop](./afk-loop/SKILL.md)** — Unattended daemon for Witify Laravel + Linear + GitHub projects: drains `ready-for-agent` Linear tickets one at a time, implementing each in an isolated worktree via `claude -p` and opening a PR for review. User-invoked.
