...
name: Witify
slug: witify
source-url: https://witify.io
created: 2026-09-10
updated: 2026-09-10
notes: Witify brand for figures embedded in witify-docx Word documents. Source: witify/skills, skills/productivity/witify-docx/template/diagram-profile/witify.md. Paper is pure white on purpose (figures land on white Word pages). Archivo replaces Instrument Serif on titles by brand decision; Helvetica with a system stack for the other text roles.
-->
# Style Guide
...
Witify skin: white paper, black ink, red accent, slate-grey muted, teal rules and links. Swap the values above and every new diagram inherits the new skin without touching any type-specific logic.
...
|---|---|---|---|
| `paper` | Page background, default node fill | `#FFFFFF` (white) | `#1D373D` (witify-teal) |
| `paper-2` | Diagram container bg, secondary fill | `#F3F4F2` | `#26444B` |
| `ink` | Primary text, primary stroke | `#000000` (black) | `#FFFFFF` (white) |
| `muted` | Secondary text, default arrow stroke | `#5E6A6E` (slate-grey) | `#B8C4C8` |
| `soft` | Sublabels, boundary labels | `#AEBDC2` | `#8A9BA1` |
| `rule` | Hairline borders | `#D0D5D3` | `rgba(255,255,255,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#1D373D` (witify-teal) | `rgba(255,255,255,0.25)` |
| `accent` | Focal / 1–2 max per diagram | `#FF4746` (witify-red) | `#FF5F5E` |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(255,71,70,0.08)` | `rgba(255,95,94,0.10)` |
| `link` | HTTP/API calls, external arrows | `#1D373D` (witify-teal) | `#8FB8C2` |

> **Brand palette source:** this skin maps to the Witify Word template (`witify-template.docx`): teal `#1D373D`, red `#FF4746`, grey `#5E6A6E`, light grey `#AEBDC2`, hairline `#D0D5D3`, surface `#F3F4F2`. The dark column is the same brand on a teal page.
...
|---|---|---|---|---|
| `title` | Archivo | 1.75rem | 700 | Page H1 |
| `node-name` | Helvetica (`Helvetica, Arial, sans-serif`) | 12px | 600 | Human-readable labels |
...
| `callout` | Helvetica *italic* (`Helvetica, Arial, sans-serif`) | 14px | 400 | Editorial asides only |

### Font stack

```html
<link href="https://fonts.googleapis.com/css2?family=Archivo:wght@700&family=Geist:wght@400;500;600&family=Geist+Mono:wght@400;500;600&family=Noto+Sans+KR:wght@400;500;600&family=Noto+Serif+KR:wght@400&family=Noto+Sans+TC:wght@400;500;600&family=Noto+Serif+TC:wght@400&display=swap" rel="stylesheet">
```
...
**Load-bearing rule:** Mono is for *technical* content (ports, commands, URLs, field types). Names go in Helvetica (system stack). Page title is Archivo 700. Italic Helvetica is reserved for annotation callouts (see [primitive-annotation.md](primitive-annotation.md)). **Never JetBrains Mono** as a blanket "dev" font.
...
- **Serif + sans + mono**: three families, not more. Witify keeps Archivo 700 for `title` and Helvetica for `callout`: the weight contrast against Helvetica text does the work the serif did.
- **Paper is pure white for Witify**: figures land on white Word pages, so the diagram must not show a tinted box. `paper-2` carries the warmth where a container needs it.
