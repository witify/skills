...
`CHANGELOG.md` is generated by [changesets](https://github.com/changesets/changesets), never edited by hand. Every user-visible change lands with a changeset — a small file in `.changeset/` naming the bump (`minor` for new or changed skills, `patch` for fixes) plus the prose that becomes its changelog entry, so write that prose for the reader. Cutting a release: push the pending changesets first (the GitHub changelog generator resolves each one's commit, and fails on commits it can't find on GitHub), run `GITHUB_TOKEN=$(gh auth token) npx changeset version`, sync the plugin version and validate as above, then commit as `chore: version skills` and push.
