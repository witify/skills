...
```bash
npx skills add witify/skills --skill=wayfinder
```
...
[Source](https://github.com/witify/skills/tree/main/skills/engineering/wayfinder)
...
The map and its tickets live on the repo's issue tracker, so wayfinder needs the tracker wiring that [setup-witify-skills](./setup-witify-skills.md) lays down — it seeds a "Wayfinding operations" section describing how the map, child tickets, blocking, and frontier queries are expressed for Linear, ClickUp, or local-markdown. Absent that doc, wayfinder defaults to a local-markdown map.
