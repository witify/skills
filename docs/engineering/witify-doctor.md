...
- **Level 0 — Machine.** Harness memory off in Claude Code and Codex (memory stores what the agent learned on *your* machine, so two developers end up with two sets of instructions neither can review; anything worth remembering belongs in the repo as a guideline). Personal `CLAUDE.md` / `AGENTS.md` / rules outside the repo, for the same reason. The `witify-skills` plugin current in both harnesses — within a release, since a fresh one lands on its own at the next session — and installed once. The MCP servers the skills need, on both sides.
