...
`python-docx` and `pymupdf` — two pip installs, no browser, so it runs wherever the agent does, macOS or Linux sandbox alike. Microsoft Word installed makes the cover render in genuine Aptos; without it the cover falls back to Helvetica. Verification needs LibreOffice.
