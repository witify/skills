...
- **[ship](./skills/engineering/ship/SKILL.md)** — Batteries-included `/implement`: one implement and one review sub-agent per ticket, then a PR to `dev` — described in simple French with high-level smoke tests — worked until CI is green and Codex approves.
